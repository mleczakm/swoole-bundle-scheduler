...
namespace SwooleBundle\Scheduler\Tests\Unit\Scheduler;
...
final class DefaultSchedulerTest extends TestCase
{
    ...
    public function testRunInvokesTheBeforeRunHookBeforePollingAndBeforeEachDispatch(): void
    {
        $message = new stdClass();
        $calls = new stdClass();
        $calls->count = 0;
        ...
        $bus = self::createStub(MessageBusInterface::class);
        $bus->method('dispatch')->willReturn(new Envelope($message));

        $scheduler = new DefaultScheduler(
            $bus,
            [self::dueOnceSchedule($message)],
            beforeRun: static function () use ($calls): void {
                $calls->count += 1;
            },
        );
        ...
        // Once before the loop, once before the single due message.
