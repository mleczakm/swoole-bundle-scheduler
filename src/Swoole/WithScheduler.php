<?php

declare(strict_types=1);

namespace SwooleBundle\Scheduler\Swoole;

use Closure;
use Psr\Log\LoggerInterface;
use Psr\Log\NullLogger;
use Swoole\Coroutine;
use Swoole\Http\Server;
use Swoole\Timer;
use SwooleBundle\Scheduler\Heartbeat\SchedulerHeartbeat;
use SwooleBundle\Scheduler\Scheduler\Scheduler;
use SwooleBundle\SwooleBundle\Bridge\Symfony\Container\CoWrapper;
use SwooleBundle\SwooleBundle\Server\Configurator\Configurator;
use Symfony\Component\Cache\LockRegistry;
use Symfony\Component\Lock\LockFactory;
use Symfony\Component\Lock\LockInterface;
use Throwable;

/**
 * Polls a {@see Scheduler} for due messages on a Swoole {@see Timer::tick()}, instead of the
 * blocking `messenger:consume scheduler_*` worker loop Symfony ships out of the box - which
 * doesn't cooperate with Swoole's event loop the way a Timer callback does.
 *
 * Every hardening feature past the bare tick loop is opt-in and stays null unless wired:
 *
 * - `$lockFactory` - a cross-process lock (a kernel-level semaphore store) around each tick, so
 *   the same tick firing in a second OS process (Swoole's manager process during a reload) does
 *   not double-dispatch. Only needed if you have actually observed duplicate dispatch.
 * - `$watchdogTimeoutSeconds` - a {@see Timer::after} deadline for one `Scheduler::run()` pass.
 *   A semaphore is not released when the coroutine holding it hangs, so a `run()` that never
 *   returns would wedge the scheduler forever; past the deadline the tick is abandoned, the
 *   lock force-released, and the next tick proceeds.
 * - `$heartbeat` - records a "last completed tick" timestamp after every successful `run()`, so
 *   an HTTP-worker health check can tell whether ticks are still completing.
 * - `$afterTick` - a no-arg callable run after every tick, success or not, for resetting
 *   app-specific state that isn't tied to a request/message boundary {@see CoWrapper::defer()}
 *   already covers.
 */
final class WithScheduler implements Configurator
{
    private const string DEFAULT_LOCK_RESOURCE = 'swoole-scheduler-tick';

    private int $tickId;

    private bool $running = false;

    /**
     * Bumped on every tick start, and again by the watchdog if it force-releases a tick.
     * tick()'s finally only cleans up when this still matches the value its own tick took,
     * so a run() that returns after the watchdog already fired doesn't double-release the
     * lock or clobber a newer tick.
     */
    private int $tickGeneration = 0;

    private readonly LoggerInterface $logger;

    private readonly ?Closure $afterTick;

    /**
     * @param positive-int $intervalMs How often, in milliseconds, to poll for due messages.
     * @param (callable(): void)|null $afterTick
     * @param positive-int|null $watchdogTimeoutSeconds
     */
    public function __construct(
        private readonly Scheduler $scheduler,
        private readonly CoWrapper $coWrapper,
        ?LoggerInterface $logger = null,
        private readonly int $intervalMs = 60_000,
        ?callable $afterTick = null,
        private readonly ?LockFactory $lockFactory = null,
        private readonly string $lockResource = self::DEFAULT_LOCK_RESOURCE,
        private readonly ?SchedulerHeartbeat $heartbeat = null,
        private readonly ?int $watchdogTimeoutSeconds = null,
    ) {
        $this->logger = $logger ?? new NullLogger();
        $this->afterTick = $afterTick !== null ? $afterTick(...) : null;
    }

    public function __destruct()
    {
        if (! isset($this->tickId)) {
            return;
        }

        Timer::clear($this->tickId);
    }

    public function configure(Server $server): void
    {
        // Symfony Scheduler's stateful Schedule::stateful()/Checkpoint::save() routes every tick
        // through Symfony Cache's LockRegistry, a pool of ~20 flock() locks on vendor PHP files.
        // Under a Swoole Timer::tick coroutine those locks have reliably wedged forever after a
        // handful of ticks (a bare cache write never returning, no error, no concurrency
        // involved); disabling file-based locking here avoids that outright. LockRegistry's
        // cross-process stampede protection buys nothing for a Timer::tick anyway - there's only
        // ever one process writing a given schedule's checkpoint. The trade-off is losing
        // stampede protection for every cache pool app-wide, not just the scheduler's, since
        // LockRegistry's state is static/global rather than scoped to one pool - acceptable since
        // it only guards redundant recomputation on a concurrent cache-miss race, not
        // correctness.
        LockRegistry::setFiles([]);

        $this->tickId = Timer::tick($this->intervalMs, $this->tick(...));

        $server->on('shutdown', function (): void {
            if (! isset($this->tickId)) {
                return;
            }

            Timer::clear($this->tickId);
        });
    }

    public function tick(): void
    {
        // Defense in depth: Timer::tick fires a new coroutine every interval regardless of
        // whether the previous tick finished. If Scheduler::run() ever stalls, this keeps
        // overlapping ticks from piling up unboundedly instead of just skipping until the stuck
        // one clears.
        if ($this->running) {
            return;
        }

        $lock = $this->acquireCrossProcessLock();

        if ($lock === false) {
            return;
        }

        $this->running = true;
        $generation = ++$this->tickGeneration;

        $watchdogId = $this->startWatchdog($generation, $lock);

        try {
            // Registers this coroutine to release/reset every pooled stateful service it
            // touches when it ends - the same mechanism request- and message-boundary handlers
            // use. Without this, nothing pooled that Scheduler::run() touches (an entity
            // manager, an event dispatcher, etc.) would ever be reset between ticks.
            //
            // Guarded by getCid(): Timer::tick's callback fires exactly once in the master
            // process during configure(), but during Server::reload() it can *also* fire once in
            // the manager process mid-reload, with no coroutine underneath it
            // (Coroutine::getCid() === -1 there). CoWrapper::defer() throws in that spillover
            // call in a way that bypasses this method's own catch below entirely and kills the
            // manager process outright, despite sitting inside this try block. getCid() is
            // documented safe to call from any context (returns -1 instead of throwing), so
            // checking first avoids ever making the call that can't be reliably caught -
            // skipping pooled-service reset for one tick is a far smaller cost than that.
            if (Coroutine::getCid() === -1) {
                $this->logger->warning(
                    'Scheduler tick has no coroutine context (likely mid worker reload/recycle) - '
                    . 'skipping pooled-service reset for this tick',
                );
            } else {
                $this->coWrapper->defer();
            }

            $this->scheduler->run();
            $this->heartbeat?->beat();
        } catch (Throwable $e) {
            // Timer::tick has no caller to propagate an exception to - nothing catches it, so
            // PHP's default uncaught-exception handling kicks in, which under Swoole's coroutine
            // hooking crashes the *entire process*, not just this coroutine. A single failed
            // tick must not be allowed to bring the whole server down; log it and let the next
            // tick retry.
            $this->logger->error('Scheduler tick failed', [
                'exception' => $e,
            ]);
        } finally {
            if ($watchdogId !== null) {
                Timer::clear($watchdogId);
            }

            if ($this->afterTick !== null) {
                ($this->afterTick)();
            }

            // Skip cleanup if the watchdog already force-released this tick (it bumps the
            // generation) or a newer tick has since taken over - releasing again would
            // over-count the semaphore or free a lock this object no longer owns.
            if ($this->tickGeneration === $generation) {
                $this->running = false;
                $lock?->release();
            }
        }
    }

    /**
     * @return LockInterface|null|false the acquired lock, null when no cross-process lock is
     *   configured, or false when another process holds it (skip this tick)
     */
    private function acquireCrossProcessLock(): LockInterface|null|false
    {
        if ($this->lockFactory === null) {
            return null;
        }

        $lock = $this->lockFactory->createLock($this->lockResource);

        try {
            if (! $lock->acquire()) {
                return false;
            }
        } catch (Throwable $e) {
            // Same reasoning as tick()'s own catch: a broken lock backend must not crash the
            // process, just skip this tick and let the next one retry.
            $this->logger->error('Scheduler tick lock acquisition failed', [
                'exception' => $e,
            ]);

            return false;
        }

        return $lock;
    }

    private function startWatchdog(int $generation, ?LockInterface $lock): ?int
    {
        if ($this->watchdogTimeoutSeconds === null) {
            return null;
        }

        // A semaphore is not released when the coroutine holding it hangs, so a Scheduler::run()
        // that never returns would keep $running true and the lock held forever, and every later
        // tick would either short-circuit on $running or fail to acquire. This watchdog fires
        // once after the deadline: if this is still the current tick and still running, it drops
        // $running and releases the lock so the next Timer::tick fire can proceed. run() itself
        // is left to finish (or not) on its own - bumping the generation makes tick()'s finally
        // a no-op for it, so there's no double release.
        return Timer::after($this->watchdogTimeoutSeconds * 1000, function () use ($generation, $lock): void {
            // A Timer::after callback has no caller either, so it swallows the same way tick()
            // does - an uncaught throw here would take the whole process down.
            try {
                if (! $this->running || $this->tickGeneration !== $generation) {
                    return;
                }

                ++$this->tickGeneration;
                $this->running = false;
                $lock?->release();

                $this->logger->error(
                    'Scheduler tick exceeded its timeout; force-released the lock so scheduling can resume',
                    ['timeout_seconds' => $this->watchdogTimeoutSeconds],
                );
            } catch (Throwable $e) {
                $this->logger->error('Scheduler watchdog failed', ['exception' => $e]);
            }
        });
    }
}
